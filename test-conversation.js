...
const conversationStateMachine = require('./src/services/conversation-state-machine/conversation-state-machine.service.ts');
...
async function testConversation() {
  ...
  await conversationStateMachine.initializeState(objjectionSession, phoneNumber);
  await conversationStateMachine.processInput(objjectionSession, 'Hello');
  await conversationStateMachine.processInput(objjectionSession, 'Yes, this is John');
  response = await conversationStateMachine.processInput(objjectionSession, 'I don\'t need a loan');
  console.log(`AI Response: "${response}"`);
